...
export default function FilteredNotificationsContainer(props: Props) {
	...
	useEffect(() => {
		if (
			...
			notificationData.pages
				...
				.filter((item) => (filter === "all" ? true : item.reason === filter)).length < 10 &&
			notificationHasNextPage
		) {
			fetchNotificationNextPage();
	...
	}, [fetchNotificationNextPage, filter, notificationData, notificationHasNextPage]);

	const { preferences } = usePreferences();
	const contentFilter = preferences?.contentFilter;

	const dataLength = notificationData?.pages.reduce((acc, page) => acc + (page?.data.notifications.length ?? 0), 0);
	...
	const isEmpty =
		...
		notificationData.pages
			.flatMap((page) => page?.data.notifications)
			.filter((item) => (filter === "all" ? true : item.reason === filter)).length === 0;
	...
	return (
		<section>
			...
			{isFetchingNotification && !isFetchingNotificationNextPage && <NotificationSkeleton />}
			{notificationError && (
			...
			{isEmpty && !notificationHasNextPage && !isFetchingMore && (
				<div className="border-skin-base border-t">
					<FeedAlert variant="empty" message="There are no notifications to show... yet" />
				</div>
			)}
			{!isEmpty &&
				!notificationError &&
