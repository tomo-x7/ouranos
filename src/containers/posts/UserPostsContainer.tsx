...
export default function UserPostsConatiner(props: Props) {
	...
	const dataLength = userPostsData?.pages.reduce((acc, page) => acc + (page?.data.feed.length ?? 0), 0);

	const isEmpty = !isFetchingUserPosts && !isFetchingUserPostsNextPage && dataLength === 0;
	...
	return (
		<div>
			...
			{!isBlocked && !hasBlockedYou && isFetchingUserPosts && !isFetchingUserPostsNextPage && (
				<FeedPostSkeleton />
			)}
			{!isBlocked && !hasBlockedYou && userPostsError && (
				<FeedAlert variant="badResponse" message="Something went wrong" />
			)}
			{isBlocked && <FeedAlert variant="empty" message={`Unblock @${handle} to view their posts`} />}
			{hasBlockedYou && <FeedAlert variant="empty" message={`@${handle}'s activity is not available`} />}
			{isEmpty && !userPostsHasNextPage && <FeedAlert variant="empty" message="There are no posts... yet" />}
			{!isEmpty &&
				!userPostsError &&
				!isFetchingUserPosts &&
				!userPostsHasNextPage &&
