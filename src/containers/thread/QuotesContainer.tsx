...
export default function QuotesContainer(props: Props) {
	...
	const dataLength = quotes?.pages.reduce((acc, page) => acc + (page?.posts.length ?? 0), 0);
		...
		<div>
			...
			>
				{quotes?.pages
					...
					.map((post, i) => (
						<Fragment key={i}>{post && <SearchPost key={i} post={post} />}</Fragment>
					))}
			</InfiniteScroll>
			...
			{isEmpty && !hasNextPage && (
				<div className="border-skin-base border-t">
					<FeedAlert variant="empty" message="No one has quoted this post... yet" />
				</div>
			)}
			{error && <FeedAlert variant="badResponse" message="Something went wrong" />}
			{!isEmpty && !error && !isFetching && !hasNextPage && !isFetchingNextPage && <EndOfFeed />}
		</div>
	);
}
