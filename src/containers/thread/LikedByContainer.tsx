...
export default function LikedByContainer(props: Props) {
	...
	const dataLength = profiles?.pages.reduce((acc, page) => acc + (page?.likes.length ?? 0), 0);
		...
		<section>
			...
			>
				{profiles?.pages
					...
					.map((profile, i) => (
						<Fragment key={i}>
							{profile && <ProfileCard key={profile?.actor.handle + i} profile={profile.actor} />}
						</Fragment>
					))}
			</InfiniteScroll>
			...
			{isEmpty && !hasNextPage && (
				<div className="px-3 md:px-0">
					<FeedAlert variant="empty" message="No one has liked this post... yet" standalone={true} />
				</div>
			)}
		</section>
	);
