...
export default function RepostedByContainer(props: Props) {
	...
	const dataLength = profiles?.pages.reduce((acc, page) => acc + (page?.repostedBy.length ?? 0), 0);
		...
		<section>
			...
			>
				{profiles?.pages
					...
					.map((profile, i) => (
						<Fragment key={i}>
							{profile && <ProfileCard key={profile.did + i} profile={profile} />}
						</Fragment>
					))}
			</InfiniteScroll>
			...
			{isEmpty && !hasNextPage && (
				<div className="px-3 md:px-0">
					<FeedAlert variant="empty" message="No one has reposted... yet" standalone={true} />
				</div>
			)}
		</section>
	);
