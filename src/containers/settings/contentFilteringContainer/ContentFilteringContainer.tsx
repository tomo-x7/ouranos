...
function Options(props: OptionsProps) {
	...
	return (
		<>
			...
			<ToggleGroupItem type="button" value={item.visibility === "ignore" ? "ignore" : "show"}>
				Show
			</ToggleGroupItem>
		</>
	);
...
export default function ContentFilteringContainer() {
	...
	const updateIsAdultContentHidden = useMutation({
		...
		mutationFn: async (value: boolean) => {
			try {
				queryClient.setQueryData(["preferences"], (oldData: PreferencesResult) => {
					return {
						...oldData,
						contentFilter: {
	...
	const updateContentFilter = useMutation({
		...
		mutationFn: async ({
			...
			try {
				queryClient.setQueryData(["preferences"], (oldData: PreferencesResult) => {
					const updatedContentFilters = oldData.contentFilter.contentFilters.map((filter) => {
						if (filter.type === pref) {
	...
	const updateAdultContentFilter = useMutation({
		...
		mutationFn: async ({
			...
			try {
				queryClient.setQueryData(["preferences"], (oldData: PreferencesResult) => {
					const updatedAdultContentFilters = oldData.contentFilter.adultContentFilters.map((filter) => {
						if (filter.type === pref) {
							return { ...filter, visibility: value };
						}
						return filter;
	...
	if (isFetchingPreferences || !preferences) return <ContentFilteringContainerSkeleton />;

	return (
		<section className="flex flex-col gap-5">
			<h2 className="text-skin-base mx-3 mb-2 text-2xl font-semibold md:mx-0">Content Filtering</h2>
			<section>
				<h3 className="text-skin-base mx-3 mb-2 text-xl font-semibold md:mx-0">Hide Adult Content</h3>
			...
			<section>
				<h3 className="text-skin-base mx-3 mb-2 text-xl font-semibold md:mx-0">Adult Content Filters</h3>
				<div>
							...
							<ToggleGroup
								...
								onValueChange={(value) => {
									...
									const updateParams = {
										...
										value: value === "ignore" ? "hide" : (value as LabelPreference),
									};
			...
			<section>
				<h3 className="text-skin-base mx-3 mb-2 text-xl font-semibold md:mx-0">Other Filters</h3>
				<div>
