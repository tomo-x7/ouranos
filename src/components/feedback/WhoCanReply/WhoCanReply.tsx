...
export default function WhoCanReply(props: Props) {
	...
	const getLabel = () => {
		if (!record.allow || record.allow.length === 0) {
			return <p className="text-skin-base text-sm">Replies to this thread are disabled</p>;
		}
		...
		for (const rule of rules) {
			if (AppBskyFeedThreadgate.isMentionRule(rule) && AppBskyFeedThreadgate.isFollowingRule(rule)) {
				mention = true;
				follow = true;
	...
	return (
		<section className="border-t-skin-base md:border-x-skin-base w-full border-x-0 border-t p-2 md:border-x">
			<div className="bg-primary/20 flex items-center gap-3 rounded-xl p-2">
				...
				<div className="flex flex-col">
					<span className="text-skin-base font-medium">{canReply ? "You can reply" : "Who can reply?"}</span>
					{getLabel()}
				</div>
			</div>
		</section>
