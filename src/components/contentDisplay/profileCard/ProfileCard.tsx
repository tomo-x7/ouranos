			...
			<article className="flex flex-col gap-2">
				<div className="flex flex-wrap justify-between gap-3">
					<div className="flex flex-wrap items-start gap-2">
						<Avatar src={profile.avatar?.replace("avatar", "avatar_thumbnail")} />
						<div className="flex flex-col">
							<div className="flex flex-wrap gap-x-1.5">
								...
								{profile.viewer?.muted ||
									(profile.viewer?.mutedByList && <ViewerInfo text="Muted user" />)}
							</div>

							{isInvalidHandle(profile?.handle) ? (
								<ViewerInfo text="Invalid Handle" />
							) : (
								<h3 className="text-skin-tertiary break-all font-medium">@{profile?.handle}</h3>
							)}
						</div>
					</div>
				</div>
				<div>
					{profile.description && (
						<p className="text-skin-base break-words leading-5">{profile.description}</p>
					)}
				</div>
			</article>
		</Link>
