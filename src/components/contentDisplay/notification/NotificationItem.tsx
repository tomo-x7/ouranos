...
import { GROUPABLE_NOTIFICATIONS, MAX_AUTHORS_SHOWN } from "@/lib/consts/notification";
...
const NotificationItem = memo(function NotificationItem(props: Props) {
	...
	const subjectUri = notification.reasonSubject as AppBskyNotificationListNotifications.Notification["reasonSubject"];
									...
									{allAuthors.slice(0, MAX_AUTHORS_SHOWN).map((author) => (
										...
										>
											<ProfileHoverCard handle={author.handle}>
												<Avatar src={author.avatar?.replace("avatar", "avatar_thumbnail")} />
											</ProfileHoverCard>
										</Link>
									))}
								...
								{allAuthors && allAuthors.length > 1 && (
									<>
										...
										<span className="text-skin-base">
											and {allAuthors.length - 1} {allAuthors.length - 1 > 1
												? "others"
												: "other"}{" "}
										</span>
									</>
