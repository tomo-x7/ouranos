...
export default function FeedHeader(props: Props) {
	...
	const { feedInfo, isLoadingFeedInfo, isFetchingFeedInfo, isRefetchingFeedInfo, feedInfoError } = useFeedInfo(feed);
	...
	const toggleLike = async () => {
		...
		if (!likeUri && feedInfo) {
			try {
				const like = await likeFeed(agent, feedInfo?.view.uri, feedInfo?.view.cid);
				setLikeUri(like?.uri);
			} catch (err) {
				setIsLiked(false);
			}
				...
				{isSaved !== null && isPinned !== null && (
					<div className="flex flex-wrap gap-3">
						...
						<Button onClick={toggleLike}>
							{likeUri && <BiSolidHeart className="text-skin-icon-like text-lg" />}
