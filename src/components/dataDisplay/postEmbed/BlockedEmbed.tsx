...
export default function BlockedEmbed(props: Props) {
	...
	return (
		<>
			{depth < 1 && (
				<div className={`border-skin-base bg-skin-base relative rounded-xl border p-3 ${replyStyle}`}>
					<div className="flex gap-2">
						<HiMiniShieldExclamation className="text-skin-icon-base text-2xl" />
						<span>Post is from a blocked user or someone who has blocked you.</span>
					</div>
					{isReply && <Threadline />}
				</div>
			)}
