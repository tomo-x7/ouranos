...
export default function TopEditorBar(props: Props) {
	const { editor, hasContent, onClose, label, onRemoveLabel, numberOfImages, onPublish } = props;
		...
		<div className="flex flex-wrap items-center justify-between gap-2">
				...
				<>
					...
					{label !== "" && (
						...
						>
							...
							{label === "nsfw" ? "Porn" : label.charAt(0).toUpperCase() + label.slice(1)}
						</Button>
					)}
					<Button
						onClick={() => {
			...
			{showCancelModal && (
				<div className="flex flex-wrap items-center justify-between gap-x-6 gap-y-2 w-full">
					<h2 className="text-skin-base text-lg font-medium">Discard this draft?</h2>
