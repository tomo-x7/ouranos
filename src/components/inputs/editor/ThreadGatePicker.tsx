...
export default function ThreadGatePicker(props: Props) {
	...
	const onSelect = (setting: ThreadgateSetting) => {
		if (setting === "nobody") {
			...
			onUpdate((prev) => {
				...
				const uniqueSettings = prev.filter((s) => s !== "nobody" && s !== setting);
	...
	const whoCanReply = selected.filter((i) => THREADGATE_OPTIONS.some((op) => op.value === i));
		...
		<Dropdown>
			...
			<Dropdown.Menu>
				<div className="px-2 pb-2 pt-1">
					<span className="text-skin-base font-semibold">Who can reply to this post?</span>
				</div>
				<Dropdown.MenuItem
					key="Everyone"
					text="Everyone"
