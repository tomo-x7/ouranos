		...
		return (
			<div className="bg-skin-base border-skin-base flex flex-wrap items-center justify-between w-full gap-3 rounded-2xl border p-3">
				<div className="flex flex-col gap-3">
					<span className="text-skin-base w-fit shrink-0">Could not get info about this link</span>
	...
	return (
		<article className="bg-skin-base border-skin-base relative rounded-2xl border">
			...
			<div className="flex flex-col p-3">
				<span className="text-skin-secondary break-all text-sm">{getHostname(link)}</span>
				{data?.title && (
					<span className="text-skin-base font-medium [overflow-wrap:anywhere]">{data.title}</span>
				)}
				{data?.description && (
					<span className="text-skin-secondary text-sm line-clamp-2">{data.description}</span>
				)}
			</div>
		</article>
	);
