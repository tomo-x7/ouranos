...
export default function Editor(props: Props) {
	...
	const [embedSuggestions, setEmbedSuggestions] = useState<Set<string>>(new Set(""));
	...
	const hasContent = !!(quote || (images && images.length > 0) || linkEmbed || richText.graphemeLength !== 0);
		...
		<section className="border-skin-base animate-fade-up animate-duration-200 bg-skin-base fixed bottom-0 z-50 h-full w-full overflow-auto rounded-t-3xl p-3 shadow-2xl md:h-auto md:max-h-[80svh] md:border-t">
			<div className="flex flex-col h-full mx-auto max-w-2xl">
				...
				<ScrollArea.Root className="max-h-[80svh] md:h-[32svh]  my-3 bg-skin-secondary p-3 rounded-2xl overflow-auto">
					...
					<TextEdit editor={editor} author={author} isReply={!!replyAuthor} />
					<div
						className={`${
							quote ||
							linkEmbed ||
					...
					>
						...
						{images && images.length > 0 && (
							<div className="my-4">
								<UploadPreview images={images.slice(0, 4)} onUpdate={setImages} />
							</div>
						)}
