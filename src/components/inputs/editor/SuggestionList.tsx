...
const SuggestionList = forwardRef<SuggestionListRef, SuggestionListProps>((props, ref) => {
	...
	return (
		<div
			className={
				"animate-fade animate-duration-300 bg-skin-base border-skin-base mb-2 mt-2 min-h-fit rounded-xl border p-1 drop-shadow-lg"
			}
		>
			{props.items.length === 0 && (
				<div className="focus:bg-skin-tertiary flex cursor-pointer items-center justify-between gap-10 rounded-md px-2 py-1 focus:outline-none">
					<span className="inline-block max-w-xs whitespace-nowrap">No User Found</span>
				</div>
			)}
			{props.items.length > 0 &&
					...
					>
						<div className="flex items-center gap-2">
							...
							<div className="flex flex-col">
								<span className="text-skin-base max-w-xs">{item.displayName || item.handle}</span>
								<span className="text-skin-tertiary text-sm">{item.handle}</span>
							</div>
						</div>
					</div>
				))}
