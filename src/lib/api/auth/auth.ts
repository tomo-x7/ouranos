...
export const authOptions: NextAuthOptions = {
	...
	callbacks: {
		...
		async session({ session, token }): Promise<Session> {
			...
			const refreshToken: { iat: number; exp: number } = jwtDecode(receivedToken.bskySession.refreshJwt);
			...
			const accessToken: { iat: number; exp: number } = jwtDecode(receivedToken.bskySession.accessJwt);
