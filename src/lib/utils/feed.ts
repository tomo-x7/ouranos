...
export function filterFeed(feed: FeedViewPost, feedFilter: FeedFilterResult) {
	...
	const isEmbed = AppBskyEmbedRecord.isView(post.embed) || AppBskyEmbedRecordWithMedia.isView(post.embed);
	const isByUnfollowed = !post.author.viewer?.following;
	const isRepost = AppBskyFeedDefs.isReasonRepost(feed?.reason);
	const isReply = feed.reply || (post.record as AppBskyEmbedRecord.View["record"]).reply;
...
export function getFeedFilter(preferences: Preferences | undefined): FeedFilterResult {
	const getFilters = (prefs?: Preferences) => {
		...
		const feedViewPref =
			(prefs.find((x) => AppBskyActorDefs.isFeedViewPref(x) && x.feed === "home") as
				| AppBskyActorDefs.FeedViewPref
				| undefined) ?? defaultFeedViewPref;
...
export function getContentFilter(preferences: Preferences | undefined): ContentFilterResult {
	const getFilters = (prefs?: Preferences) => {
		...
		Object.entries(prefs).map(([_, pref]) => {
			...
			if (isContentPref) {
				...
				const filter = CONTENT_FILTER_OPTIONS.find((f) => !f.adult && f.type === label);
				if (filter) {
	...
	const sortedContentFilters = filters.contentFilters.sort((a, b) => a.label.localeCompare(b.label));

	const sortedAdultContentFilters = filters.adultContentFilters.sort((a, b) => a.label.localeCompare(b.label));
...
export default function getThreadPreferences(preferences: Preferences | undefined) {
	const getFilters = (prefs?: Preferences) => {
		...
		Object.entries(prefs).map(([_, pref]) => {
			...
			if (isThreadViewPref) {
				threadViewPrefs.prioritizeFollowedUsers =
					pref.prioritizeFollowedUsers ?? THREAD_VIEW_PREFS.prioritizeFollowedUsers;
...
export const getPostFilter = (post: AppBskyFeedDefs.FeedViewPost, filter: ContentFilterResult) => {
	...
	const embedRecordLabel = (post?.post?.embed?.record as ViewRecord)?.record as ViewRecord;
	...
	const message =
		adultContentFilters.find((f) => f.values.includes(label || embedLabel))?.message ||
	...
	const shouldHide = isAdultContentHidden ? true : !!(visibility === "hide" || visibility === "warn");

	const showToggle = !!(visibility !== "show" && visibility !== "ignore" && (label || embedLabel));
...
export const getThreadPostFilter = (post: AppBskyFeedDefs.PostView, filter: ContentFilterResult) => {
	const { isAdultContentHidden, adultContentFilters } = filter;

	const label = post.labels?.map((l) => l.val)[0] ?? ""; // ex. "nsfw", "suggestive"
	const embedLabel = post.embed?.record
		? ((post.embed.record as ViewRecord)?.labels?.map((l) => l.val)[0] ?? "")
		: "";

	const message =
		adultContentFilters.find((f) => f.values.includes(label || embedLabel))?.message ?? "Marked content";
